...
module Database
  ...
  class Model
    ...
    def self.all
      Database::Model.execute("SELECT * FROM #{self.table}").map do |row|
        self.new(row)
      end
    end
    ...
    def self.where(query, *args)
      Database::Model.execute("SELECT * FROM #{self.table} WHERE #{query}", *args).map do |row|
        self.new(row)
      end
    end
